...
function callGemini(prompt, modelName) {
  ...
  const resolvedModel = modelName || config.MODEL_NAME || "models/gemini-2.5-flash-latest";
  const modelPath = String(resolvedModel).startsWith("models/") ? resolvedModel : `models/${resolvedModel}`;
